      ...
      int fromRank, int toRank ) 
{
   ...
   printf("sendims %d %d, size/n", sendDim[0], sendDim[1], sizeof(float));
      ...
      {  
         ...
         // to call your sobel filtering code on each tile
            ...
            if(t->tileRank == 0) {
               ...
               do_sobel_filtering(t->inputBuffer.data(), t->outputBuffer.data(), t->newWidth, t->newHeight);
            }
...
scatterAllTiles(int myrank, vector < vector < Tile2D > > & tileArray, float *s, int global_width, int global_height)
{
   ...
   for (int row=0;row<tileArray.size(); row++)
   {
      ...
      {  
         ...
         if(t->xloc+t->width>= global_width && xloc == 0) width = (t->width);
         int height = (t->height) + 2;
         if(t->yloc+t->height >= global_height || yloc ==0) height = (t->height)+1;
         if(t->yloc+t->height >= global_height && yloc ==0) height = (t->height);
         t->newWidth = width;
         t->newHeight = height;
